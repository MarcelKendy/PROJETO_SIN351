...
int fifo(int8_t** page_table, int num_pages, int prev_page,
    ...
    // Acha a pagina que contem a moldura mais velha e a retorna
    int i;
...
int second_chance(int8_t** page_table, int num_pages, int prev_page,
                  int fifo_frm, int num_frames, int clock) {
    // Percorre-se todas as paginas de cada modulo/frame desde o inicio e se o frame for o respectivo e tiver o bit 0,
    // retorna a pagina, se nao, seta como 1 e joga no final da lista usando o prev_page, ( creio que se usaria ele...)
    // Caso os bits estejam dando no mesmo, faz algoritmo do fifo normal.
...
int nru(int8_t** page_table, int num_pages, int prev_page,
        ...
        // Acho as paginas da classe 0, se estiver disponivel, a retorno, caso não, repito o processo pra classe menos prioritaria ate a ultima.
        for (int page = 0; page < num_pages; page++) {
...
int aging(int8_t** page_table, int num_pages, int prev_page,
    ...
    // Acha a página em primeira posicão que possui mapeamento e depois acha a página com o a aging menor a partir do primeiro endereco
    ...
    int beginning = page + 1;
    for(i = beginning; i < num_pages; i++) {

        if (page_table[i][PT_MAPPED] != 0) {

            if (page_table[i][PT_AGING_COUNTER] < page_table[age_aux][PT_AGING_COUNTER]) {
...
int random_page(int8_t** page_table, int num_pages, int prev_page,
                int fifo_frm, int num_frames, int clock) {
    //printf("%d\n%d\n%d\n%d\n%d\n%d\n",*page_table,num_pages,prev_page,fifo_frm,num_frames,clock);
